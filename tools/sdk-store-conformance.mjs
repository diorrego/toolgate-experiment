...
const mutation=process.env.TOOLGATE_TEST_MUTATION==='1';
...
const tool={tool_id:'read.value',name:'read.value',title:'Read value',description:'Read a synthetic value',aliases:[],tags:[],effect:mutation?'write':'read',open_world:false,input_schema:{type:'object',properties:{name:{type:'string'}},required:['name'],additionalProperties:false},schema_profile:'tg-jsonschema-1'};
let uncertain=false;
const registry=new ToolRegistry([{metadata:tool,handler:async(args)=>{appendFileSync(state.counter,'dispatch\n',{mode:0o600});if(uncertain)throw Error('Synthetic lost business response');return{content:[{type:'text',text:JSON.stringify({value:args.name})}]};}}],undefined,{allowMutations:mutation});
const metrics=[];
...
let authorization=true;let approval=true;let authorizations=0;
const provider=new ToolgateProvider({catalogId:'sdk-fixture',registry,client,store,authorize:async()=>{authorizations++;return authorization;},approve:async()=>approval});
const execute=(...args)=>mutation?provider.executeWrite(...args):provider.executeRead(...args);
const count=()=>{try{return readFileSync(state.counter,'utf8').trim().split('\n').filter(Boolean).length;}catch{return 0;}};
try{
 if(childMode){try{const result=await execute(state.actor,state.input);console.log(JSON.stringify({ok:true,result}));}catch(error){console.log(JSON.stringify({ok:false,code:error.code??'UNKNOWN'}));}}
 else{
  ...
  if(mutation){const noApproval=new ToolgateProvider({catalogId:'sdk-fixture',registry,client,store,authorize:async()=>true});await assert.rejects(noApproval.executeWrite(state.actor,input));await assert.rejects(provider.executeRead(state.actor,input));approval=false;await assert.rejects(execute(state.actor,input));assert.equal(count(),0);approval=true;}
  const result=await execute(state.actor,input);assert.equal(count(),1);assert.deepEqual(await execute(state.actor,input),result);assert.equal(count(),1);
  await assert.rejects(execute({...state.actor,principal_ref:'synthetic-other'},input));assert.equal(count(),1);
  await assert.rejects(execute(state.actor,{...input,arguments:{name:'changed'}}));assert.equal(count(),1);
  if(mutation){approval=false;await assert.rejects(execute(state.actor,input));assert.equal(count(),1);approval=true;}
  authorization=false;await assert.rejects(execute(state.actor,input));authorization=true;assert.equal(count(),1);
  ...
  const repaired=await execute(state.actor,{operation_id:invalid.operation_id,operation_revision:invalid.revision,arguments:{name:'repaired'}});assert.ok(repaired.content);assert.equal(count(),2);
  ...
  if(mutation){const pending=await provider.prepare(state.actor,'write value',{name:'uncertain'});const pendingInput={operation_id:pending.operation_id,operation_revision:pending.revision,arguments:{name:'uncertain'}};uncertain=true;await assert.rejects(execute(state.actor,pendingInput));uncertain=false;assert.equal(count(),4);await assert.rejects(execute(state.actor,pendingInput));assert.equal(count(),4);}
  console.log(JSON.stringify({passed:true,mutation,writeChecks:mutation?['default_approval_denied','read_path_rejects_write','approval_before_dispatch','approval_before_replay','uncertain_write_not_retried']:[],checks:['jev_usage_over_http','prepare_no_dispatch','local_dispatch','durable_replay','actor_isolation','argument_binding','revoked_authorization','needs_arguments','repair_without_reselection','process_restart_replay','two_process_claim'],handler_invocations:count(),authorizations,child_outcomes:children.map(c=>({ok:c.ok,code:c.code}))},null,2));
 }
