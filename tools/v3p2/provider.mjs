...
export async function createV3Provider({
  config,
  catalog,
  gateTools,
  callNative,
  ...
  const actor = {
    principal_ref: "v3-synthetic-operator",
    workspace_ref: "v3-synthetic-company",
    authorization_revision: "v3-sandbox-policy-1",
        ...
        },
        {
          onRequest: (m) =>
            record.remote?.push({
              ...m,
              hops: m.hops.map((h) => {
                const t = h.workflowTrace;
                const ids = registry.ids();
                const names = (indices) =>
                  indices === null ? null : indices.map((i) => ids[i]);
                return {
                  ...h,
                  toolStages:
                    t &&
                    t.catalog_version === registry.version &&
                    [
                      ...t.retrieved,
                      ...(t.included ?? []),
                      ...(t.exposed ?? []),
                    ].every((i) => i < ids.length)
                      ? {
                          retrieved: names(t.retrieved),
                          included: names(t.included),
                          exposed: names(t.exposed),
                        }
                      : null,
                };
              }),
  ...
  function reset(nextMode) {
    if (
      !["direct", "joint-1", "joint-3", "joint-5", "joint-8"].includes(nextMode)
    )
  ...
  async function dispatch(name, args) {
    ...
    const provider = connections.get("choice").provider;
    let value;
    if (name === "prepare_workflow") {
      const event = {
        remoteStart: record.remote.length,
        remoteEnd: null,
        exposed: [],
        status: null,
        error: null,
      };
      record.preparationEvents.push(event);
      try {
        value = await provider.prepareWorkflow(actor, args.intent, {
          exposureLimit: Number(mode.split("-")[1]),
        });
        event.status = value.status;
        event.exposed = value.operations.map((op) => op.selected_tool.tool_id);
      } catch (error) {
        event.error = error.code ?? error.message;
        throw error;
      } finally {
        event.remoteEnd = record.remote.length;
      }
      record.preparations.push(value);
      record.selectionProposals.push(
        ...value.operations.map((op) => op.selected_tool.tool_id),
      );
  ...
  async function mcp(req, res, body) {
    const server = new Server(
      { name: "woku-v3-laboratory", version: "3.0.0" },
      { capabilities: { tools: {} } },
    );
