...
import { executeCalls } from "./calls.mjs";
...
if (
  ...
  throw Error("V3 requires 30 English cases and all 143 active tools");
...
const planned = smoke
  ? schedule([corpus.cases[0], corpus.cases[21]])
...
try {
  for (const task of planned) {
    ...
    record.grade = grade(record, oracle, oracleIds);
    record.grade.state = verifyState(
      oracle,
      baselineState,
      record.finalState,
      oracleIds,
    );
    record.grade.workflowSuccess =
      record.grade.completeSequence &&
      record.grade.groundedIds &&
      record.grade.state.correct &&
      record.grade.wrongMutationCalls === 0;
    record.grade.endToEndSuccess =
      record.grade.workflowSuccess && record.status === "completed";
    ...
    console.log(
      JSON.stringify({
        ...
        coverage: record.grade.initialCompleteCoverage,
        workflow: record.grade.workflowSuccess,
